    ...
    public partial class CustomerController : AdminControllerBase
    {
        ...
        protected CustomerModel PrepareCustomerModelForList(Customer customer)
        {
            return new CustomerModel
            {
                ...
                Company = customer.AffiliateId.ToString(),
				...
				PlacementId = customer.PlacementId,
		...
		protected virtual void PrepareCustomerModelForCreate(CustomerModel model)
		{
			...
			model.CoinPaymentEnabled = coinpaymentSettings.CP_IsActivePaymentMethod;

			var SolidTrustPaySettings = _services.Settings.LoadSetting<SolidTrustPaySettings>(storeScope);
			model.STPEnabled = SolidTrustPaySettings.STP_IsActivePaymentMethod;

			var PayzaSettings = _services.Settings.LoadSetting<PayzaSettings>(storeScope);
			model.PayzaEnabled = PayzaSettings.PZ_IsActivePaymentMethod;

			var PMSettings = _services.Settings.LoadSetting<PMSettings>(storeScope);
			model.PMEnabled = PMSettings.PM_IsActivePaymentMethod;

			var PayeerSettings = _services.Settings.LoadSetting<PayeerSettings>(storeScope);
			model.PayeerEnabled = PayeerSettings.PY_IsActivePaymentMethod;

			model.UPIPaymentEnabled = true;
		...
		protected virtual void PrepareCustomerModelForEdit(CustomerModel model, Customer customer)
		{
			...
			model.AllowManagingCustomerRoles = _permissionService.Authorize(StandardPermissionProvider.ManageCustomerRoles);
			model.AllowManageCustomers = _permissionService.Authorize(StandardPermissionProvider.ManageCustomers);
        ...
        public ActionResult List()
        {
			...
			var customers = _customerService.GetAllCustomers(null, null, new int[] { registeredRoleId }, null,
                ...
                false, null, 0, _adminAreaSettings.GridPageSize,null);
        ...
        public ActionResult Create(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
				...
				_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.StokistCommision, model.StokistCommision);
				...
				if (coinpaymentSettings.CP_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.BitcoinAddressAcc, model.BitcoinAddressAcc);
				if (coinpaymentSettings.CP_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.BankName, model.BankName);
				if (coinpaymentSettings.CP_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.NICR, model.NICR);
				if (coinpaymentSettings.CP_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.AccountNumber, model.AccountNumber);
				if (coinpaymentSettings.CP_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.AccountHolderName, model.AccountHolderName);

				var SolidTrustPaySettings = _services.Settings.LoadSetting<SolidTrustPaySettings>(storeScope);
				if (SolidTrustPaySettings.STP_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.SolidTrustPayAcc, model.SolidTrustPayAcc);

				var PayzaSettings = _services.Settings.LoadSetting<PayzaSettings>(storeScope);
				if (PayzaSettings.PZ_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.PayzaAcc, model.PayzaAcc);

				var PMSettings = _services.Settings.LoadSetting<PMSettings>(storeScope);
				if (PMSettings.PM_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.PMAcc, model.PMAcc);

				var PayeerSettings = _services.Settings.LoadSetting<PayeerSettings>(storeScope);
				if (PayeerSettings.PY_IsActivePaymentMethod)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.PayeerAcc, model.PayeerAcc);

				if (model.UPIPaymentEnabled)
					_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.UPIPaymentNumber, model.UPIPaymentNumber);
                ...
                return continueEditing ? RedirectToAction("Edit", new { id = customer.Id }) : (AllowList ? RedirectToAction("List") : RedirectToAction("Index","Home"));
            }
        ...
        public ActionResult Edit(int id)
        {
            ...
            var model = new CustomerModel
			{
				...
				IsCountryManager = customer.IsCountryManager,
			...
			model.Company = customer.AffiliateId.ToString();
			...
			model.BitcoinAddressAcc = customer.GetAttribute<string>(SystemCustomerAttributeNames.BitcoinAddressAcc);
			...
			model.UPIPaymentNumber = customer.GetAttribute<string>(SystemCustomerAttributeNames.UPIPaymentNumber);
        ...
        public ActionResult Edit(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
					...
					customer.IsCountryManager = model.IsCountryManager;
                    //email
                    if (!String.IsNullOrWhiteSpace(model.Email))
                    {
		...
		{
			TempReferralList model = new TempReferralList();
			...
			for(int i = 1; i <= 20; i++)
			{
				...
				model.AvailableLevels.Add(item);
			}

			model.TotalCommission = _customerService.GetCustomerCommission(_workContext.CurrentCustomer.Id);
			return View(model);
		}

		[HttpPost, GridAction(EnableCustomBinding = true)]
		public ActionResult ListMyReferral(GridCommand command,int LevelId)
		{
			var gridModel = new GridModel<TempReferralList>();
			var customerReferral = _customerService.GetCustomerReferral(_workContext.CurrentCustomer.Id).Where(x=>x.LevelId == LevelId).ToList();
		...
		[HttpPost]
		public ActionResult PifMember(string email,int amount,int levelid)
		{
			try
			{
				var plan = _boardService.GetBoardById(amount);
				var repurchasebalance = _customerService.GetAvailableBalance(_workContext.CurrentCustomer.Id);
				var amountreq = Convert.ToInt64(plan.Price) * 1;
				...
				ReleaseLevelCommission(customer, plan.Id,plan.Price);
				

				
				NotifySuccess("Your PIF was successfull");
				return Json(new
				{
					Success = true
				});
		...
		public void ReleaseLevelCommission(Customer customer, int boardid,decimal amountreq)
		{
			//Save board position
			try
			{
			...
			{
				//WritetoLog("Position Purchase error :" + ex.ToString());
			}
		}
